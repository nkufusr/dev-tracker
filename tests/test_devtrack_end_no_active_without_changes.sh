...
set +e
output="$("$DEVTRACK_REPO/scripts/devtrack-end.sh" "no-op smoke" 2>&1)"
status=$?
set -e

test "$status" -ne 0
echo "$output" | grep -q "没有活跃会话。请先运行 'devtrack 开始'"
if echo "$output" | grep -q "是否创建补录会话并继续结束"; then
  echo "unexpected backfill prompt without changes" >&2
  exit 1
fi
