...
如果执行 `devtrack 结束` 时发现没有活跃会话，`devtrack` 会先检查“最近一次已完成会话”的基线和当前工作区之间是否存在真实文件变更。

- 如果没有真实变更，仍然保持原来的报错：`没有活跃会话。请先运行 'devtrack 开始'`
- 如果检测到真实变更，会提示是否创建一个补录会话，并继续执行这次 `devtrack 结束`
- 确认补录后，新会话会继承最近一次已完成会话的基线，并在 `session.yaml` 中记录 `backfill_from`
